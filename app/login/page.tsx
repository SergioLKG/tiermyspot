...
export default function LoginPage() {
  ...
  if (error && (error.includes("403") || error.includes("Forbidden"))) {
    error =
      "No tienes acceso a esta aplicación. Por favor, contacta con un administrador para ser añadido como tester.";
  }
  ...
  const handleDemoLogin = async () => {
    ...
    try {
      ...
      if (result?.error) {
        console.error("Error en inicio de sesión demo:", result.error);
      } else {
        router.push("/dashboard");
      }
    } catch (error) {
          ...
          <CardContent className="space-y-4">
            {error && (
              <Alert variant="destructive" className="mb-4">
                ...
                <AlertDescription>
                  {error === "OAuthSignin" &&
                    "Error al conectar con Spotify. Inténtalo de nuevo."}
                  {error === "OAuthCallback" &&
                    "Error al procesar la respuesta de Spotify."}
